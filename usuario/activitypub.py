...
def get_actor_url(user):
    domain = get_valor('dominio')
    return f"https://{domain}/usuario/{user.username}"
...
def send_follow_activity(user, target_actor_url):
    ...
    activity = {
        ...
        "id": f"{actor_url}#follow/{target_actor_url}"
    }

    # Get the inbox URL from the target actor
    try:
        response = requests.get(target_actor_url, 
                              headers={'Accept': 'application/activity+json'})
        response.raise_for_status()
        actor_info = response.json()
        inbox_url = actor_info.get('inbox')
        
        if not inbox_url:
            raise ValueError("No inbox URL found in actor info")
...
def search_users_remote(query):
    """Search for users on remote instances"""
    # Implementation of remote user search
    # This is just a placeholder - you'll need to implement the actual federation logic
    return []

def process_follow_request(data):
    ...
    try:
        ...
        # Verify this is for one of our users
        domain = get_valor('dominio')
        if not target.startswith(f"https://{domain}/usuario/"):
            return False
        ...
        send_accept_activity(actor, data)
        return True
...
def send_accept_activity(actor, follow_activity):
    """Send an Accept activity in response to a Follow"""
    try:
        # Get actor info to find inbox
        ...
        if not inbox_url:
            raise ValueError("No inbox URL found")
        ...
        print(f"Error sending Accept activity: {e}")
        return False 